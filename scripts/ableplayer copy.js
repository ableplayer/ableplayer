/* 
  JavaScript for Univeral Media Player (UMP)
  http://www.terrillthompson.com/ump
*/

// Construct a UniversalMediaPlayer object 
// Parameters are: 
// mediaId - the id attribute of the UMP container div 
// umpIndex - the index of this UMP instance (if page includes only one UMP, umpIndex = 0) 
// startTime - the time at which to begin playing the media       
function UniversalMediaPlayer(mediaId, umpIndex, startTime) { 
  ...
  // useFallback - set to true to force browser to use the fallback player 
  // recommended for testing fallback player only 
  ...
  if (mediaId) { 
    ...
    this.$umpDiv = $('#' + mediaId);    
    if ((this.$umpDiv).find('audio').length > 0) { 
      // this.$media is a JQuery object pointing to audio element
      this.$media = this.$umpDiv.find('audio').first(); 
      // this.media is the HTML5 audio object itself
      this.media = this.$media[0];  
      this.mediaType = 'audio';       
    }
    else if ((this.$umpDiv).find('video').length > 0) { 
      // this.$media is a JQuery object pointing to audio element
      this.$media = (this.$umpDiv).find('video').first();
      // this.media is the HTML5 video object itself
      this.media = this.$media[0];  
      this.mediaType = 'video';         
    }
    else { // no media found
      if (this.debug) {       
        console.log('No audio or video element found within #' + mediaId);
      }
    }
    if (this.mediaType) { 
    
      // this is either a <video> or <audio> element
      this.$mediaContainer = $('#' + mediaId + ' div.ump-media-container');         
      this.$playerDiv = $('#' + mediaId + ' div.ump-player');
      this.$playerDiv.addClass('ump-'+this.mediaType);
      this.$nowPlayingDiv = $('#' + mediaId + ' div.ump-now-playing');
      this.$controllerDiv = $('#' + mediaId + ' div.ump-controller');
      this.$statusBarDiv = $('#' + mediaId + ' div.ump-status-bar');
      this.$timer = $('#' + mediaId + ' span.ump-timer');
      this.$status = $('#' + mediaId + ' span.ump-status'); 
      ...
      if (this.$tracks.length > 0) { 
        if (this.mediaType == 'video') { 
          // also get container that captions or description will be appended to
          this.$vidcapContainer = $('#' + mediaId + ' div.ump-vidcap-container'); 
        }
        ...
        for (var i=0; i<this.$tracks.length; i++) { 
          ...
          if (kind == 'captions') { 
            ...
            this.captions = new Array(); //temp container for storing data from source file
            ...
            this.hasClosedDescription = true;
            ...
            this.description = new Array(); //temp container for storing data from source file
            this.currentDescription = -1;
            if (this.prefDesc && this.prefClosedDesc) { 
...
console.log('about to init descrip dude...');
      this.initDescription(); 
      
      // initialize playlist  (assumes playlist is nested inside media div)   
      if ($('#' + this.mediaId + ' ul.ump-playlist').length > 0) { 
        this.hasPlaylist = true;        
        this.$playlist = $('#' + this.mediaId + ' ul.ump-playlist li');
        // add tabindex to each list item 
        this.$playlist.attr('tabindex','0');
        this.playlistSize = this.$playlist.length;
      ...
      if (this.browserCanPlay()) { 
        if (this.useFallback || (this.isUserAgent('msie 9') && this.mediaType == 'video')) {
          // this browser can play HTML5, but...
          // the user wants JW, or  
          ...
          // Couldn't figure out a solution to this problem - IE10 fixes it. Meanwhile...             
          this.player = 'jw'; 
        }
        else { 
          this.player = 'html5';
      ...
      if (this.player == 'html5') { 
        ...
        // JW Player uses 0 to 100 for volume. Need to convert default
        this.volume = this.volume * 100; 
          
        // get vars from HTML5 code, and use them to initialize jwplayer 
        if (this.mediaType == 'video') {
          var poster = this.$media.attr('poster');
          var jwHeight = this.$media.attr('height');
          var jwWidth = this.$media.attr('width');
          if (this.debug) { 
            console.log ('retrieved height (' + jwHeight + ') and width (' + jwWidth + ') from video element');
          }
          if ((jwHeight == undefined) || (jwWidth == undefined)) { 
            // use defaults (defined at top of this file)
            var jwHeight = this.videoHeight;
            var jwWidth = this.videoWidth;
          }           
          var $jwSource = this.$media.find('source[type="video/mp4"]');
          var captionFile = this.$media.find('track[kind="captions"]').attr('src');
        }
        else { 
          for (var i=0; i<this.$sources.length; i++) { 
            var sourceType = this.$sources[i].getAttribute('type'); // ???
            if (sourceType == 'audio/aac') { 
              this.mediaFile = this.$sources[i].getAttribute('src');
              break;
            }
            else if (sourceType == 'audio/mpeg') { 
              this.mediaFile = this.$sources[i].getAttribute('src');
            }
          }
        }
        if (this.mediaFile) { // something was found that JW can play 
          ...
          // there is no media file that JW PLayer can play 
          // should toss up an error message 
        }       
      } // end else use JW Player

      // the following apply to both HTML5 and JW players 
      ...
      this.initializing = false;
    } // end if there's a media type
  } // end if there's a mediaId
} 
UniversalMediaPlayer.prototype.getDimensions = function() { 
...
UniversalMediaPlayer.prototype.setButtons = function() { 
...
UniversalMediaPlayer.prototype.initDescription = function() { 
  ...
  if (this.prefDesc == 1) { 
    ...
    else if (this.hasOpenDescription == true) { 
      // only open description is available
      this.useDescType = 'open';
      this.openDescOn = true;
      this.closedDescOn = false;
    }
    else if (this.hasClosedDescription == true) { 
  ...
  if (this.debug) { 
    console.log('hasOpenDescription: ' + this.hasOpenDescription);
    console.log('hasClosedDescription: ' + this.hasClosedDescription);
...
UniversalMediaPlayer.prototype.addPrefsForm = function() { 
...
UniversalMediaPlayer.prototype.addHelp = function() {   
...
UniversalMediaPlayer.prototype.setCookie = function(cookieValue) { 
  if ($.isFunction($.cookie)) { 
    // set cookie that expires in 90 days 
    $.cookie('Universal-Media-Player',cookieValue,90);  
  }
}
UniversalMediaPlayer.prototype.getCookie = function() { 
  if ($.isFunction($.cookie)) { 
    return $.cookie('Universal-Media-Player');
  }
}
UniversalMediaPlayer.prototype.setPrefs = function() { 
  ...
  if (cookie) { 
    if (cookie.length == cookieLength) { 
      for (var i=0; i<cookieLength; i++) { 
        var thisPref = this.prefs[i]['name'];
        var thisValue = cookie.substr(i,1);
...
UniversalMediaPlayer.prototype.savePrefs = function() { 
...
UniversalMediaPlayer.prototype.setupAlert = function() { 
...
UniversalMediaPlayer.prototype.showAlert = function(msg) { 
  this.$alertBox.text(msg).dialog('open');
}
UniversalMediaPlayer.prototype.browserCanPlay = function() { 
...
UniversalMediaPlayer.prototype.isUserAgent = function(which) {
...
UniversalMediaPlayer.prototype.addEventListeners = function() { 

  // Save the current object context in thisObj for use with inner functions.
  var thisObj = this;
    
  // handle clicks on player buttons 
  $('#' + this.mediaId + ' .ump-controller input').click(function(){
  ...
  if (this.player == 'html5') {
    ...
    jwplayer(thisObj.jwId)
      .onTime(function() { 
        ...
        if (thisObj.duration == undefined || thisObj.duration == -1) { 
...
UniversalMediaPlayer.prototype.addControls = function() {   
  ...
  if (this.mediaType == 'video') { 
    ...
    if (this.hasOpenDescription || this.hasClosedDescription) { 
  ...
  if (this.mediaType == 'video') { 
    ...
    totalWidth += 10;
console.log('setting width to ' + totalWidth);        
...
UniversalMediaPlayer.prototype.handlePlay = function(e) { 
  if (this.player == 'html5') {       
...
UniversalMediaPlayer.prototype.handleStop = function() { 
  if (this.player == 'html5') {             
    // reset media
    this.media.pause(); 
...
UniversalMediaPlayer.prototype.handleRewind = function() { 
  if (this.player == 'html5') {             
...
UniversalMediaPlayer.prototype.handleForward = function() { 
  if (this.player == 'html5') {             
    var targetTime = this.media.currentTime + this.seekInterval;    
...
UniversalMediaPlayer.prototype.handleMute = function() { 
  if (this.player == 'html5') {             
...
UniversalMediaPlayer.prototype.handleVolume = function(direction) {   
...
UniversalMediaPlayer.prototype.handleSpeed = function(direction) { 
...
UniversalMediaPlayer.prototype.handleCaptionToggle = function() { 
...
UniversalMediaPlayer.prototype.handleDescriptionToggle = function() { 
  ...
  else if (this.hasOpenDescription) { 
    // only open description is available
    if (this.openDescOn) { 
       // open description is on. Turn it off (swap to non-described video)
      this. openDescOn = false;
  ...
  else if (this.hasClosedDescription) { 
    // only closed description is available
...
UniversalMediaPlayer.prototype.handleSettingsClick = function() { 
  $('.ump-prefs-form').dialog('open');  
}
UniversalMediaPlayer.prototype.handleHelpClick = function() { 
  $('.ump-help-div').dialog('open');
}
UniversalMediaPlayer.prototype.addSeekControls = function(leftPos) { 
  if (this.useSlider) {
    var sliderWidth = 200;
...
UniversalMediaPlayer.prototype.getButtonTitle = function(control) { 
...
UniversalMediaPlayer.prototype.seekTo = function (newTime) { 
...
UniversalMediaPlayer.prototype.updateTime = function(whichTime, time) {
...
UniversalMediaPlayer.prototype.setupTimedText = function(kind,track) {  
...
UniversalMediaPlayer.prototype.showCaptions = function() { 
  if (this.player == 'html5') {
...
UniversalMediaPlayer.prototype.showDescription = function() { 
console.log('showDescription has been called');
...
UniversalMediaPlayer.prototype.swapDescription = function() { 
...
UniversalMediaPlayer.prototype.swapSource = function(sourceIndex) { 
...
UniversalMediaPlayer.prototype.toSeconds = function(t) {
  var s = 0.0
  if (t) {
    var p = t.split(':');
...
UniversalMediaPlayer.prototype.strip = function(s) { 
  if (s) { 
    return s.replace(/^\s+|\s+$/g,"");
  }
}
UniversalMediaPlayer.prototype.playAtTime = function(seconds) { 
  //seeking = true;
  //seekVideo(seconds);
}
UniversalMediaPlayer.prototype.highlightTranscript = function (currentTime) { 
...
UniversalMediaPlayer.prototype.usingModifierKeys = function(e) { 
