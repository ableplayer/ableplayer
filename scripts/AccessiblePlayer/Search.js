export default class Search{
    ...
    showSearchResults$() {
        ...
        if (this.ablePlayer.searchDiv && this.ablePlayer.searchString) {
            if ($('#' + this.ablePlayer.SearchDiv)) {
                var resultsArray = this.searchFor(this.ablePlayer.searchString);
                if (resultsArray.length > 0) {
                    ...
                    for (var i = 0; i < resultsArray.length; i++) {
                        ...
                        itemStartSpan.click(function (event) {
                            ...
                            spanStart += .01;
                            thisObj.ablePlayer.seeking = true;
                    ...
                    $('#' + this.ablePlayer.searchDiv).append(resultsSummary, resultsList);
                } else {
                    var noResults = $('<p>').text('No results found.');
                    $('#' + this.ablePlayer.searchDiv).append(noResults);
                }
            }
        }
    };
    ...
    searchFor (searchString, ignoreCaps) {
        var captionLang, captions, results, caption, c, i, j;

        // split searchTerms into an array
        var searchTerms = searchString.split(' ');
        if (this.ablePlayer.captions.length > 0) {
            captionLang = this.ablePlayer.captions[0].language; // in case it's needed later
            captions = this.ablePlayer.captions[0].cues;
            if (captions.length > 0) {
                var results = [];
                c = 0;
                for (i = 0; i < captions.length; i++) {
                    if ($.inArray(captions[i].components.children[0]['type'], ['string', 'i', 'b', 'u', 'v', 'c']) !== -1) {
                        caption = this.ablePlayer.caption.flattenCueForCaption(captions[i]);
    ...
    highlightSearchTerm (searchTerms, resultString) {
