export default class AccessibleDialog{
    ...
    constructor(  modalDiv, $returnElement, dialogRole, title, $descDiv, closeButtonLabel, width, fullscreen, escapeHook ) {
        ...
        if (!fullscreen) {
            var closeButton = $('<button>',{
                ...
                'title': thisObj.able.closeButtonLabel,
        ...
        modal.keydown(function (e) {
            ...
            // Tab
                ...
                var focusable = parts.filter(this.focusableElementsSelector).filter(':visible');
    ...
    show() {
        ...
        var focusable = this.modal.find("*").filter(this.focusableElementsSelector).filter(':visible');
        if (focusable.length === 0) {
            this.ablePlayer.focusedElementBeforeModal.blur();
        }
        setTimeout( () => {
            // set focus on the first focusable element
            this.modal.find("*").filter(this.focusableElementsSelector).filter(':visible').first().addClass('focus-visible');
            this.modal.find("*").filter(this.focusableElementsSelector).filter(':visible').first().focus();
        }, 300);
    };
